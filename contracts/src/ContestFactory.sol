...
contract ContestFactory is Ownable {
    ...
    mapping(bytes32 => ContestData.PlayerPerformance[]) public contestPlayerPerformances;
    ...
    mapping(bytes32 => ContestData.PrizeBreakdown[]) public contestPrizeBreakdowns;
    ContestData.PrizeBreakdown[] public defaultPrizeBreakdown;
    
    // Events
    event ContestCreated(bytes32 indexed contestId, string matchId, uint256 entryFee);
    event TeamSubmitted(bytes32 indexed contestId, address indexed user, uint256 teamIndex);
    event ScoresFinalized(bytes32 indexed contestId, string matchId);
    event PrizesDistributed(bytes32 indexed contestId, address[] winners, uint256[] amounts);
    ...
    function createContest(
        ...
        require(matchContestCount[matchId] < MAX_CONTESTS_PER_MATCH, "Max contests per match reached");
        ...
        require(contests[contestId].startTime == 0, "Contest ID already exists");
    ...
    function submitTeam(
        ...
        require(userContestEntries[contestId][msg.sender] == 0, "Already submitted a team");
        require(playerIds.length == MIN_PLAYERS, "Team must have exactly 11 players");
        ...
        for (uint256 i = 0; i < playerIds.length; i++) {
            if (keccak256(abi.encodePacked(playerIds[i])) == keccak256(abi.encodePacked(captainId))) {
                captainFound = true;
            }
            if (keccak256(abi.encodePacked(playerIds[i])) == keccak256(abi.encodePacked(viceCaptainId))) {
        ...
        require(
            keccak256(abi.encodePacked(captainId)) != keccak256(abi.encodePacked(viceCaptainId)), 
        ...
        userContestEntries[contestId][msg.sender] = contestTeams[contestId].length;
        ...
        emit TeamSubmitted(contestId, msg.sender, contestTeams[contestId].length - 1);
    }
     ...
     */
    function updateContestStatus(bytes32 contestId, ContestData.MatchStatus status) external onlyOwner {
    ...
    function requestMatchData(bytes32 contestId) external payable onlyOwner {
        ...
        fdcDataConsumer.requestMatchScorecard{value: msg.value}(contest.matchId, cricketApiKey);
    }
     ...
     */
    function finalizeScores(bytes32 contestId, IJsonApi.Proof calldata proof) external onlyOwner {
        ContestData.Contest storage contest = contests[contestId];
        require(contest.startTime > 0, "Contest does not exist");
        require(!contest.scoresFinalized, "Scores already finalized");
        
        // Process the FDC attestation proof
        ContestData.MatchScorecard memory scorecard = fdcDataConsumer.processAttestationProof(
            contest.matchId,
            proof
        );
        ...
        for (uint256 i = 0; i < scorecard.playerPerformances.length; i++) {
            contestPlayerPerformances[contestId].push(scorecard.playerPerformances[i]);
        }
    ...
    function distributePrizes(bytes32 contestId) external onlyOwner {
        ...
        ContestData.UserTeam[] storage teams = contestTeams[contestId];
        ContestData.PrizeBreakdown[] storage prizeBreakdown = contestPrizeBreakdowns[contestId];
        ...
        for (uint256 i = 0; i < prizeBreakdown.length; i++) {
            ...
            uint256 prizeAmount = (contest.prizePool * prizeBreakdown[i].percentage) / 10000;
    ...
    function setContestPrizeBreakdown(
        ...
        require(contests[contestId].startTime > 0, "Contest does not exist");
        require(contests[contestId].participantCount == 0, "Cannot change after participants joined");
     ...
     */
    function updateCricketApiKey(string memory _cricketApiKey) external onlyOwner {
        cricketApiKey = _cricketApiKey;
    }
     ...
     */
    function getContest(bytes32 contestId) external view returns (ContestData.Contest memory) {
     ...
     * @return User's team
     */
    function getUserTeam(bytes32 contestId, address userAddress) external view returns (ContestData.UserTeam memory) {
     ...
     */
    function getAllContestTeams(bytes32 contestId) external view returns (ContestData.UserTeam[] memory) {
     ...
     */
    function getPlayerPerformances(bytes32 contestId) external view returns (ContestData.PlayerPerformance[] memory) {
     ...
     */
    function getMatchContests(string memory matchId) external view returns (bytes32[] memory) {
