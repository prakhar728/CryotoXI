...
contract RetrieveProofScript is Script {
    ...
    function run() external {
        ...
        console.log("Retrieved request bytes and voting round ID:", votingRoundId);
        ...
        ParsableProof memory proofData = abi.decode(dataJson, (ParsableProof));
        
        console.log("Attestation type:", vm.toString(proofData.attestationType));
        console.log("Proofs length:", proofData.proofs.length);
        
        // Format the proof for the contract
        IJsonApi.Proof memory proof = IJsonApi.Proof({
            proofs: proofData.proofs,
            data: IJsonApi.Response({
                responseBody: IJsonApi.ResponseBody({
                    abi_encoded_data: proofData.responseHex
                })
            })
        });
        ...
        vm.writeFile(
            ...
            vm.serializeBytes32("proof", "proofs", proofData.proofs)
        );
        ...
        contestFactory.finalizeScores(contestId, proof);
